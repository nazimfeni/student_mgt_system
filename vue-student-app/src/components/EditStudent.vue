  ...
  <div class="max-w-md mx-auto mt-10 p-6 border rounded-xl shadow bg-white">
    ...
    <form @submit.prevent="updateStudent" class="space-y-4">
      <div>
        ...
        <input v-model="student.name" type="text" class="w-full p-2 border rounded" />
      </div>

      <div>
        <label class="block text-sm font-medium">Email</label>
        <input v-model="student.email" type="email" class="w-full p-2 border rounded" />
      </div>

      <div>
        <label class="block text-sm font-medium">Class</label>
        <input v-model="student.class" type="text" class="w-full p-2 border rounded" />
      </div>
...
const updateStudent = async () => {
  try {
    await axios.put(`http://localhost:8000/api/students/${studentId}`, student.value)
